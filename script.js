...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (graphForm) {
    graphForm.addEventListener('submit', (e) => {
      ...
      setAIInsights('ai-insights', 'Insights will appear here.');
    });
...
function setAIInsights(sectionClass, textOrAnalysis) {
  ...
  if (typeof textOrAnalysis === 'string') {
    container.innerHTML = `<p>${textOrAnalysis}</p>`;
    return;
  }
...
function normalizeTextToTwoColumnCSV(text) {
  ...
  const dataPattern = /^(\d{4})\s+([-+]?\d*\.?\d+)$/; // year value
  let started = false;
  const extracted = [];
  let considered = 0;
  for (const line of lines) {
    ...
    const partsCsv = csvLike ? line.split(',').map(s => s.trim()) : null;
    const wsParts = line.split(/\s+/).map(s => s.trim());
    // Detect start: either year value by whitespace, or two CSV cells with numeric second
    const match = line.match(dataPattern);
    const twoCells = (partsCsv && partsCsv.length >= 2 && /^\d{4}$/.test(partsCsv[0]) && /^[-+]?\d*\.?\d+$/.test(partsCsv[1])) ||
                     (wsParts.length >= 2 && /^\d{4}$/.test(wsParts[0]) && /^[-+]?\d*\.?\d+$/.test(wsParts[1]));
    if (!started && (match || twoCells)) started = true;
    ...
    if (twoCells) {
      ...
      const val = partsCsv ? partsCsv[1] : wsParts[1];
...
// Patch Create Graph page to call analysis after graphing
if (graphForm) {
  graphForm.addEventListener('submit', (e) => {
    setTimeout(() => {
      const csv = document.getElementById('dataInput').value.trim();
      showAIAnalysis(csv);
    }, 100); // after chart renders
  });
}
// Patch Compare Data page to call analysis after graphing
if (compareForm) {
  compareForm.addEventListener('submit', (e) => {
    setTimeout(() => {
      const csv1 = document.getElementById('dataset1').value.trim();
      const csv2 = document.getElementById('dataset2').value.trim();
      let text = '';
      if (typeof analyzeDataset === 'function') {
        const analysis1 = analyzeDataset(csv1);
        const analysis2 = analyzeDataset(csv2);
        text = `Dataset 1 Analysis:\n${analysis1.plain_text}\n\nDataset 2 Analysis:\n${analysis2.plain_text}`;
      } else {
        text = 'AI analysis module not loaded.';
      }
      setAIInsights('ai-insights', text);
    }, 100); // after chart renders
  });
}